  ...
  const handleArchive = () => {
    ...
    setTimeout(() => {
      const newPost: Post = {
        ...
        category: '感悟',
        tags,
        coverImage: coverImage || `https://images.unsplash.com/photo-1441974231531-c6227db76b6e?q=80&w=1200&sig=${Date.now()}`,
      ...
      <div className="max-w-5xl mx-auto flex flex-col lg:flex-row gap-12">
        <div className="flex-1 space-y-12 animate-fade-in-up">
          <div className="glass p-6 rounded-[2rem] shadow-sm border border-accent/5 flex flex-col md:flex-row gap-4 items-center">
            <div className="flex items-center space-x-3 shrink-0">
              <div className="w-8 h-8 bg-accent/10 rounded-xl flex items-center justify-center">
                <svg className={`w-4 h-4 text-accent ${isGenerating ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">灵感捕捉</span>
            </div>
            <input 
              type="text" 
              value={topic}
          ...
          <div className="flex space-x-8 border-b border-gray-100">
            ...
            >
              编写模式
              {activeTab === 'edit' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent"></div>}
            </button>
            <button 
            ...
            >
              预览模式
              {activeTab === 'preview' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent"></div>}
            </button>
          </div>
          ...
          {activeTab === 'edit' ? (
            <div className="space-y-8 min-h-[600px]">
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map((tag, i) => (
                  <span key={i} className="px-3 py-1 bg-accent/5 text-accent text-[10px] font-bold rounded-full border border-accent/10 flex items-center group">
                    #{tag}
                    <button onClick={() => setTags(tags.filter((_, idx) => idx !== i))} className="ml-2 opacity-0 group-hover:opacity-100 hover:text-red-500 transition-opacity">
            ...
            <article className="prose prose-stone max-w-none animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <h1 className="font-serif font-black">{title || "未命名文章"}</h1>
              <div className="flex gap-2 mb-8">
                {tags.map(tag => <span key={tag} className="text-accent text-xs font-bold">#{tag}</span>)}
              </div>
        ...
        <aside className="lg:w-72 shrink-0">
          <div className="sticky top-32 space-y-6">
            <div className="glass p-8 rounded-[2.5rem] border border-accent/5 shadow-xl relative overflow-hidden group">
              ...
              <div className="mb-8">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-primary mb-4">封面图设置</h3>
                <div 
                  onClick={() => coverInputRef.current?.click()}
                  className={`relative aspect-video rounded-2xl border-2 border-dashed border-gray-100 flex flex-col items-center justify-center cursor-pointer hover:border-accent/40 hover:bg-accent/5 transition-all overflow-hidden ${coverImage ? 'border-solid' : ''}`}
                >
                  {coverImage ? (
                    <img src={coverImage} className="w-full h-full object-cover" alt="Selected Cover" />
                  ) : (
                    <>
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-gray-300">上传文章封面</span>
                    </>
                  )}
                  {coverImage && (
                    <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 flex items-center justify-center transition-opacity">
                      <span className="text-white text-[9px] font-black uppercase tracking-widest">更换封面</span>
                    </div>
                  )}
                </div>
              </div>
  ...
  >
    <div className="flex items-center justify-between mb-1">
      <div className="flex items-center space-x-3">
        ...
        <span className="text-[11px] font-black uppercase tracking-wider text-primary group-hover:text-accent transition-colors">{label}</span>
      </div>
